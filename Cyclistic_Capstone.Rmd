...
I will try to change it to a timezone that does not observe daylight savings, but the time was originally captured in Central timezone. 
There might not be a difference but I want to try it.

```{r timezone change}
# used to check the attributes of started_at and time_started
attributes(bikes_2021_v4$started_at)  # NULL

attributes(bikes_2021_v4$time_started) # POSIXct and UTC
```

It already is in UTC and the initial data provided had no timezone associated.

So I change that timezone into central and then changed it to UTC.  
...
Now we do the same to the ending times.
...
So just looking at it.  The time conversion happened but the time differences are still there.  We still have times that are behind other times.
Let's go ahead and calculate the duration anyway.
...
We still have negative duration.  Let's look at the time to double check those numbers

```{r daylight savings check}
bikes_2021_v4 %>% 
...
Those issues are still there because the time was recorded in Central.  This is how we get ride of those unneeded columns.

```{r drop time coloums}
# drops the cited columns
...
We can delete those entries or we can go in and subtract or add time to the duration only. I'm leaning toward deleting entries because we do have other negative entries in other months because some bike stations are not accurately timed.
...
We know that those rides are gone but let's double check.  If done correctly, What should pull up are rides that last 1 minute.
